...
const RejectedActivity = () => {
    ...
    // Always set status to 'rejected' for this component
    const getInitialStatusFromRoute = () => {
        return 'rejected';
    };

    // Status filter is always disabled and set to rejected
    ...
    // Filter state - status is always 'rejected'
    ...
    const fetchData = async () => {
        await Promise.all([
            fetchActivities(true), // Always apply filters for rejected
            fetchPromoters(),
            fetchRoutePlans(),
        ]);
    };
    ...
    const handleExportPPT = async () => {
        ...
        try {
            ...
            const params = new URLSearchParams();
            params.append('status', 'rejected'); // Always rejected for this component
            if (stateFilter !== 'all') params.append('state', stateFilter);
            if (districtFilter !== 'all') params.append('district', districtFilter);
            if (subDistrictFilter !== 'all') params.append('sub_district', subDistrictFilter);
            if (villageFilter !== 'all') params.append('village', villageFilter);
            ...
            let filename = 'rejected_activities_presentation.pptx';
    ...
    const handleExportActivity = async () => {
        ...
        try {
            ...
            let filename = 'rejected_activities_export.csv';
            const contentDisposition = response.headers.get('Content-Disposition');
            if (contentDisposition) {
                const filenameMatch = contentDisposition.match(/filename="(.+)"/);
                if (filenameMatch) {
    ...
    const handleResetFilters = () => {
        setStatusFilter('rejected'); // Keep status as rejected
        setStateFilter('all');
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Activities that have been rejected ({totalActivities} total)
                    </p>
                </div>
            ...
            <div className="bg-gradient-to-r from-card via-card/95 to-card shadow-lg rounded-xl border border-border/50 backdrop-blur-sm">
                ...
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${
                    ...
                    <div className="p-6">
                        ...
                        {/* Status Filter - Disabled and shown as Rejected */}
                        <div className="space-y-2">
                            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Status</label>
                            <Select value="rejected" disabled>
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Showing {table.getFilteredRowModel().rows.length} of {totalActivities} rejected activities
                            </CardDescription>
                        </div>
